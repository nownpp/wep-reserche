...
export async function POST(req: Request) {
  try {
    ...
    if (stage === 'outline' && username) {
      ...
      try {
        ...
        if (!contentType || !contentType.includes('application/json')) {
          ...
          // سنحاول الاستمرار إذا كان الخطأ غير حاسم أو إعطاء رسالة واضحة
        ...
        // نجح الخصم، نحصل على الرصيد الجديد
    ...
    // توجيه إلزامي للالتزام بخطة مستخدم في حال إرسالها
    const outlineInstruction = outlineContext 
      ? `\n\nتنبيه هام جداً: يجب أن تلتزم التزاماً حرفياً بتسلسل العناوين الموجود في خطة البحث المعتمدة التالية أثناء كتابتك لهذا الجزء:\n\n[خطة البحث المعتمدة]\n${outlineContext}\n[/نهاية خطة البحث]`
      : '';
...
هام جداً: في الصفحة الأخيرة والمخصصة للمصادر، يجب كتابة المصادر بالصيغة الأكاديمية التالية: (اسم المؤلف، وسنة النشر، وعنوان المقال أو الكتاب، والصفحة أو الصفحات التي تم الاقتباس منها).` + noMarkdown;
    ...
    if (!response.ok) {
      ...
      return NextResponse.json(
        { error: `عذراً، فشل الاتصال بمحرك الذكاء الاصطناعي. يرجى المحاولة مرة أخرى لاحقاً.` },
        { status: 500 }
      );
    }
